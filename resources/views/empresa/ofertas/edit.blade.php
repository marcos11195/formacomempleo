<x-app-layout>
    ...
    <form method="POST"
        ...
        <div class="text-center">
            <h1 class="text-2xl font-bold text-gray-800">Editar oferta</h1>
            <p class="text-gray-600 mt-1">Modifica los datos de esta oferta profesional.</p>
        </div>
                ...
                <div>
                    ...
                    <select id="idpuesto" name="idpuesto"
                        ...
                        <option value="{{ $puesto->id }}"
                            {{ old('idpuesto', $oferta->idpuesto) == $puesto->id ? 'selected' : '' }}>
                            {{ $puesto->nombre }}
                        </option>
                        @endforeach
                    </select>
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <textarea id="requisitos" name="requisitos"
                        class="mt-1 block w-full border-gray-300 rounded-md"
                        rows="4">{{ old('requisitos', $oferta->requisitos) }}</textarea>
                </div>
                ...
                <div>
                    ...
                    <textarea id="funciones" name="funciones"
                        class="mt-1 block w-full border-gray-300 rounded-md"
                        rows="4">{{ old('funciones', $oferta->funciones) }}</textarea>
                </div>
                ...
                <div>
                    ...
                    <select id="idsector" name="idsector"
                        ...
                        <option value="{{ $sector->id }}"
                            {{ old('idsector', $oferta->idsector) == $sector->id ? 'selected' : '' }}>
                            {{ $sector->nombre }}
                        </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select id="idmodalidad" name="idmodalidad"
                        ...
                        <option value="{{ $modalidad->id }}"
                            {{ old('idmodalidad', $oferta->idmodalidad) == $modalidad->id ? 'selected' : '' }}>
                            {{ $modalidad->nombre }}
                        </option>
                        @endforeach
                    </select>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <x-input id="salario_min" name="salario_min" type="number" step="0.01"
                        class="mt-1 block w-full"
                        value="{{ old('salario_min', $oferta->salario_min) }}" />
                </div>
                ...
                <div>
                    ...
                    <x-input id="salario_max" name="salario_max" type="number" step="0.01"
                        class="mt-1 block w-full"
                        value="{{ old('salario_max', $oferta->salario_max) }}" />
                </div>
        ...
        <div class="bg-white p-6 rounded-lg shadow space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div>
                    <x-label for="fecha_publicacion" value="Fecha de publicación (opcional)" />
                    <x-input id="fecha_publicacion" name="fecha_publicacion" type="date"
                        class="mt-1 block w-full"
                ...
                <div>
                    ...
                    <x-input id="publicar_hasta" name="publicar_hasta" type="date"
                        class="mt-1 block w-full"
                        value="{{ old('publicar_hasta', $oferta->publicar_hasta?->format('Y-m-d')) }}" />
                </div>
                ...
                <div>
                    ...
                    <x-input id="fecha_incorporacion" name="fecha_incorporacion" type="date"
                        class="mt-1 block w-full"
                        value="{{ old('fecha_incorporacion', $oferta->fecha_incorporacion?->format('Y-m-d')) }}" />
                </div>
        ...
        <div class="bg-white p-6 rounded-lg shadow space-y-6">
            ...
            <select id="estado" name="estado"
                ...
                <option value="{{ $estado }}"
                    {{ old('estado', $oferta->estado) == $estado ? 'selected' : '' }}>
                    {{ ucfirst($estado) }}
                </option>
                @endforeach
            </select>
